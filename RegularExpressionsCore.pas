...
begin
  ...
  CleanUp;
  FPattern := pcre_compile(MarshaledAString(UTF8String(FRegEx)), FPCREOptions, @Error, @ErrorOffset, FCharTable);
      ...
      '\':
        begin
          ...
          case Char(S[J]) of
            ...
            'g':
              begin
                if (J < Length(S)-2) and (S[J+1] = '<') and (S[J+2] in ['A'..'Z', 'a'..'z', '_']) then
                begin
                  // Python-style named group reference \g<name>
                  J := J+3;
                  while (J <= Length(S)-1) and (S[J] in ['0'..'9', 'A'..'Z', 'a'..'z', '_']) do
                    Inc(J);
...
begin
  Result := UTF8ToString(FReplacement);
end;
...
begin
  ...
  if Index > GroupCount then
    ...
    Result := UTF8ToString(Copy(FSubject, Offsets[Index*2]+1, Offsets[Index*2+1]-Offsets[Index*2]));
end;
...
begin
  Result := UTF8ToString(GetSubjectLeftUTF8);
end;
...
begin
  Result := UTF8ToString(GetSubjectRightUTF8);
end;
...
//      Inc(Offsets[I]);
    ...
    if Offsets[0] = Offsets[1] then // Make sure we don't get stuck at the same position
      if (0 <= Offsets[0]) and (Offsets[0] < Length(FSubject)) then
        Inc(FStart, UTF8Len(FSubject[Offsets[0]]))
      else
        Inc(FStart);
    if Assigned(OnMatch) then
      OnMatch(Self)
...
begin
  Result := pcre_get_stringnumber(FPattern, MarshaledAString(UTF8String(Name)));
end;
...
begin
  ...
  begin
    ...
    Count := 1;
    repeat
      Strings.Add(UTF8ToString(Copy(FSubject, Offset+1, InternalGetMatchedOffset - Offset)));
      Inc(Count);
      Offset := InternalGetMatchedOffset + InternalGetMatchedLength ;
    until ((Limit > 1) and (Count >= Limit)) or not MatchAgain;
    Strings.Add(UTF8ToString(Copy(FSubject, Offset+1, Length(FSubject) - Offset)));
  end
end;
  ...
  begin
    ...
    repeat
      ...
      begin
        ...
        begin
          LUseOffset := False;
          Strings.Add(UTF8ToString(Copy(FSubject, 1, InternalGetMatchedOffset)));
      ...
      begin
        Strings.Add(UTF8ToString(Copy(FSubject, LOffset+1, InternalGetMatchedOffset - LOffset)));
        Inc(Count);
        if Self.GroupCount > 0 then
          Strings.Add(Groups[GroupCount]);
      end;
    ...
    until ((Limit > 1) and (Count >= Limit)) or not MatchAgain;
    Strings.Add(UTF8ToString(Copy(FSubject, LOffset+1, Length(FSubject) - LOffset)));
  end
end;
